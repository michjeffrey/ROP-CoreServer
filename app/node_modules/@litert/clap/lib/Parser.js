...
exports.ClapParser = void 0;
const E = require("./Errors");
const _ = require("./Utils");
const ParserContext_1 = require("./ParserContext");
class ClapParser {
    constructor(_opts, _rules, _helpGen) {
        this._opts = _opts;
        this._rules = _rules;
        this._helpGen = _helpGen;
    }
    parse(args) {
        const ctx = new ParserContext_1.ParserContext(this._rules, this._opts, args);
        do {
            ...
            switch (ctx.status) {
                case ParserContext_1.EParserStatus.READING_COMMAND: {
                ...
                case ParserContext_1.EParserStatus.READING_ARGUMENTS: {
                    if (this._tryParseTailingArguments(p, ctx)) {
                        break;
                    }
                    if (!this._opts.disableFlagsAfterArguments || !ctx.countArguments()) {
                        if (this._tryParseOptionLongAssignMode(p, ctx)
                            || this._tryParseFlagLongMode(p, ctx)
                            || this._tryParseOptionShortAssignMode(p, ctx)
                            || this._tryParseOptionShortAttachMode(p, ctx)
                            || this._tryParseFlagShortMode(p, ctx)) {
                ...
                case ParserContext_1.EParserStatus.READING_OPTION_ARG: {
    ...
    _tryParseFlagShortMode(piece, ctx) {
        if (_.isShortFlagMixedExpr(piece)) {
            ...
            if (optNames.length) {
                ctx.status = ParserContext_1.EParserStatus.READING_OPTION_ARG;
    ...
    _tryParseFlagLongMode(piece, ctx) {
    ...
    _tryParseOptionShortAttachMode(piece, ctx) {
        if (_.isShortAttachLikeExpr(piece)
    ...
    _tryParseOptionShortAssignMode(piece, ctx) {
    ...
    _tryParseOptionLongAssignMode(piece, ctx) {
    ...
    _tryParseTailingArguments(piece, ctx) {
        if (piece === '--') {

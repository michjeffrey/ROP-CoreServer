...
router.notFound(async function (ctx) {
    ...
    response.write(`URL Path: ${request.path}<br>`);
    response.write(`Handler Path: ${request.realPath}<br>`);
    response.write(`Search: ${request.queryString}<br>`);
    response.write(`Request Host: ${request.host}<br>`);
    response.write(`Request Domain: ${request.hostDomain}<br>`);
    response.write(`Request Port: ${request.hostPort}<br>`);
    response.write(`Server Host: ${request.server.host}<br>`);
    response.write(`Server Port: ${request.server.port}<br>`);
    response.write(`SSL: ${request.https ? "Yes" : "No"}<br>`);
    response.write(`Remote IP: ${request.ip}<br>`);
    response.write(`Date: ${new Date(request.time).toUTCString()}<br>`);
    request.loadCookies();
...
server.start();
//# sourceMappingURL=https-2.js.map
...
const Plain_1 = require("./router-rules/Plain");
const RegExp_1 = require("./router-rules/RegExp");
const Smart_1 = require("./router-rules/Smart");
class Middleware {
}
class Router {
    constructor() {
        ...
        // @ts-ignore
        this._middlewares = {};
        for (let method of Abstract_1.HTTP_METHODS) {
            this._middlewares[method] = [];
        }
        this._notFoundHandler = async (ctx) => {
            ctx.response.writeHead(Abstract_1.HTTPStatus.NOT_FOUND, "FILE NOT FOUND");
            ctx.response.end();
        };
    }
    _isSmartRule(path) {
    ...
    _setupMiddlewareRule(middleware, path) {
        if (typeof path === "string") {
            ...
            if (this._isSmartRule(path)) {
                middleware.rule = new Smart_1.default(null, path, {});
            }
            else {
                middleware.rule = new Plain_1.default(null, path, {});
            }
        }
        else if (path instanceof RegExp) {
            middleware.rule = new RegExp_1.default(null, path, {});
        }
    }
    use(...args) {
        ...
        switch (args.length) {
            case 1:
                ...
                break;
            case 2:
                if (Abstract_1.HTTP_METHODS.indexOf(arg0) > -1) {
    ...
    register(method, path, handler, data = {}) {
        ...
        if (path instanceof RegExp) {
            this._addRegExpRule(method, new RegExp_1.default(handler, path, data));
        }
            ...
            if (this._isSmartRule(path)) {
                this._addRegExpRule(method, new Smart_1.default(handler, path, data));
            }
            else {
                collection[path] = new Plain_1.default(handler, path, data);
            }
        }
        return this;
    }
...
exports.createStandardRouter = createStandardRouter;
exports.default = createStandardRouter;

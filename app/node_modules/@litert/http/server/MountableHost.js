...
class MountableHost extends AbstractServer_1.default {
    constructor(opts) {
        super(opts);
        this._mounts = opts.mounts;
        for (let prefix in this._mounts) {
            ...
            for (let k in this._opts.plugins) {
                if (!this._mounts[prefix].hasPlugin(k)) {
                    this._mounts[prefix].setPlugin(k, 
                    // @ts-ignore
    ...
    __requestEntry(context) {
        ...
        for (let prefix in this._mounts) {
            if (request.realPath.startsWith(prefix)) {
                ...
                // @ts-ignore
                request.connection.server[Internal_1.cServer] = this._mounts[prefix];
                // @ts-ignore
                request.plugins = response.plugins = this._opts.plugins;
                return this._mounts[prefix].__requestEntry(context);
            }
        }
        return super.__requestEntry(context);
    }
}
exports.MountableHost = MountableHost;
exports.default = MountableHost;
//# sourceMappingURL=MountableHost.js.map
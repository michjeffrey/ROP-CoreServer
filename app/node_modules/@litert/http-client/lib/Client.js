...
const SimpleKVSCache_1 = require("./SimpleKVSCache");
const I = require("./Internal");
const Helper_1 = require("./Internal/Helper");
class HttpClient {
    constructor(opts) {
        this.filters = opts?.filters ?? observable_1.Filters.createFilterManager();
        this._kvCache = opts?.kvCache ?? (0, SimpleKVSCache_1.createSimpleKVSCache)(C.DEFAULT_PROTOCOL_DETECTION_CACHE_TTL);
        this._ = new Helper_1.HttpHelper();
        this._clients = {
    ...
    async request(optsIn) {
        /**
         * Specify whether requires the entity of request ornot.
         */
        const REQ_ENTITY = this._.requireEntity(optsIn.method);
        ...
        function _default(obj, key, defaultValue) {
    ...
    async _autoDetectProtocol(opts) {
        const tlsOpts = {
        ...
        return new Promise((resolve, reject) => {
            const conn = $TLS.connect({
                ...
                if (conn.alpnProtocol === 'h2') {
                    const key = this._clients.h2s.getAuthorityKey(opts);
                     ...
                     */
                    conn.secureConnecting = false;
                    resolve(this._wrapResponse(this._clients.h2s.request(opts, conn, key)));
                }
                else if (conn.alpnProtocol === 'http/1.1') {
                    const key = this._clients.h2s.getAuthorityKey(opts);
                    this._kvCache.set(key, 'h1s');
                    resolve(this._wrapResponse(this._clients.h1s.request(opts, conn, key)));
                }
                else {
                    conn.destroy();
    ...
    async _wrapResponse(pResult) {
        const result = await pResult;
        return new I.HttpClientResponse(result.protocol, result.stream, result.contentLength, result.headers, result.statusCode, result.gzip, result.deflate, result.noEntity);
    }
}

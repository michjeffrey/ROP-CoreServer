...
function sign(algo, key, message) {
    return $Crypto.createHmac(algo, key).update(message).digest();
}
exports.sign = sign;
function verify(algo, key, message, signature) {
    return sign(algo, key, message).compare(signature) === 0;
}
exports.verify = verify;
function signStream(algo, key, message) {
...
async function verifyStream(algo, key, message, signature) {
    return signature.compare(await signStream(algo, key, message)) === 0;
}
exports.verifyStream = verifyStream;
function createSigner(algo, key) {
    const ret = {};
    Object.defineProperties(ret, {
        sign: {
            writable: false,
            configurable: false,
            value: (msg) => sign(algo, key, msg)
        },
        signStream: {
            writable: false,
            configurable: false,
            value: (msg) => signStream(algo, key, msg)
        },
        verify: {
            writable: false,
            configurable: false,
            value: (msg, sign) => verify(algo, key, msg, sign)
        },
        verifyStream: {
            writable: false,
            configurable: false,
            value: (msg, sign) => verifyStream(algo, key, msg, sign)
        },
        hashAlgorithm: {
            writable: false,
            configurable: false,
